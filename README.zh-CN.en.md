...
1. 从 [Releases](https://github.com/ru-gong/OpenRemoteAssistant/releases) 下载 `OpenRemoteAssistant-<version>-development.pkg` 和 `SHA256SUMS.txt`。
...
先在目标软件中选择“遥控器麦克风”，再在助手的“语音软件”菜单选择预设。“触发方式”可选点按或长按，“快捷键”默认 Fn，可改为左右 Command / Control / Option / Shift、F1–F20、空格或 Return。目标软件须设置相同的按键和模式；例如点按 Command。切换软件预设会更新建议模式、保留按键。旧设置升级后保持 Fn。预设只需要辅助功能权限。这里发送的是 macOS 软件 Fn，不保证只读取原始 HID 的软件能够识别。
...
RC003-MS 麦克风进入 Typeless 的基本链路已实测可用；新增自选快捷键组合、其他语音软件预设、全部实体按键和干净 Mac 安装仍需继续验证。
...
1. Download `OpenRemoteAssistant-<version>-development.pkg` and `SHA256SUMS.txt` from [Releases](https://github.com/ru-gong/OpenRemoteAssistant/releases).
...
Select `遥控器麦克风` in the target application first, then choose its preset in the assistant. Choose tap-to-toggle or hold-to-talk under “触发方式” and the target key under “快捷键”. Fn is the default; alternatives include left/right Command, Control, Option, Shift, F1–F20, Space and Return. Set the same key and mode in the target app. Choosing another app preset updates the suggested mode but keeps the selected key. Existing settings migrate with Fn unchanged. Presets require Accessibility permission only. The assistant emits a macOS software Fn event, which applications that read raw HID only may ignore.
...
- Version: 0.2.10 (build 17), development test build.
...
The basic RC003-MS microphone-to-Typeless path has been tested successfully. Custom key/mode combinations, other voice-app presets, every physical button, and clean-Mac installation still need further validation.
